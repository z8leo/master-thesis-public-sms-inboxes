...
# Which networks are used for Austrian and German phone numbers?
# Filter dataset
df = df[df['original_country_name_hlr'].str.contains('Germany|Austria', na=False)]
df = df.reset_index()
# When a number is ported, the serving network is 'ported_network_name'
sns.set(rc={"figure.figsize":(14, 25)}) #width=3, #height=4
# Build Dataframe, when is_ported -> ported network name, else original_network_name
...
ax = sns.countplot(df['network'], x=df['network'], order=df['network'].value_counts().index)
ax.set_xticklabels(ax.get_xticklabels(), rotation=90)
ax.set_xlabel('Serving Network names for Germany and Austria')
ax.set_ylabel('Occurences')
ax.bar_label(ax.containers[0])
plt.tight_layout()
plt.show()